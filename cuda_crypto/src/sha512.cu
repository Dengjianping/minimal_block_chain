...
__global__ void sha512(uint_64* d) {
    //int index = blockDim.y * blockIdx.y + threadIdx.y;

    __shared__ uint_64 temp[80];
    uint_64 h[8]; // long long takes 2 register files
#pragma unroll
...
#pragma unroll
    ...
    reinterpret_cast<ulonglong4*>(d)[0] = reinterpret_cast<ulonglong4*>(h)[0];
    reinterpret_cast<ulonglong4*>(d)[1] = reinterpret_cast<ulonglong4*>(h)[1];
}

void paddle_bits_512(const std::string &_input) {
    std::string c = "";
    uint_64 N[16] = { 0 };
...
extern "C" const char* SHA512(const char *input) {
    paddle_bits_512(std::string(input));
    dim3 block_size(1, 1);
    dim3 grid_size(1);

    uint_64 *d_s;
    cudaMalloc(&d_s, sizeof(uint_64) * 8);

    sha512 <<<1, 1>>> (d_s);
    cudaDeviceSynchronize();

    uint_64 h_s[8] = { 0 };
    cudaMemcpy(h_s, d_s, sizeof(uint_64) * 8, cudaMemcpyDeviceToHost);
    cudaFree(d_s);

    std::stringstream stream;
    for (unsigned int i = 0; i < 8; ++i) {
        stream << std::hex << h_s[i];
    }
